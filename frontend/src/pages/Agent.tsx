...
function StepCard({ step, t }: { step: Step; t: Translate }) {
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-3">
      ...
      {step.thought && <p className="mt-1.5 ps-7 text-xs italic text-slate-500">{step.thought}</p>}
      <p className="mt-1.5 whitespace-pre-wrap ps-7 text-sm text-slate-700">{step.observation}</p>
        ...
        <div className="mt-2 flex flex-wrap gap-1.5 ps-7">
          {step.sources.map((s, i) => (
            ...
            >
              ...
              {s.label || t("ag.sourceRef", { type: enumLabel(s.type, t), id: s.id ?? "" })}
            </span>
          ))}
        </div>
      )}
...
function SkillCard({
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setDeleteError(err.status === 409 ? t("ag.skillHasHistory") : err.message);
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-3">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            <Badge tone={skill.status === "active" ? "green" : "slate"}>{enumLabel(skill.status, t)}</Badge>
          </div>
        ...
        {isAdmin && (
          <div className="flex shrink-0 gap-1">
            <button
              ...
              disabled={busy}
              aria-label={skill.status === "active" ? t("ag.deprecateSkill") : t("ag.reactivateSkill")}
              title={skill.status === "active" ? t("ag.deprecate") : t("ag.reactivate")}
            ...
            <button
              ...
              aria-label={t("ag.deleteSkillAria")}
      ...
      <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500">
        <span>{t("ag.steps", { n: skill.plan.length })}</span>
        <span>{t("ag.used", { n: skill.usage_count })}</span>
        <span>{t("ag.success", { n: Math.round(skill.success_rate * 100) })}</span>
        {skill.parameters.length > 0 && <span>{t("ag.params", { p: skill.parameters.join(", ") })}</span>}
      </div>
      ...
      {confirming && (
        <Modal title={t("ag.deleteSkillTitle")} onClose={() => setConfirming(false)}>
          <div className="space-y-4">
            <p className="text-sm text-slate-600">
              {t("ag.confirmDeleteSkill", { name: skill.name })}
            </p>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button variant="danger" disabled={busy} onClick={confirmDelete}>
                {busy ? t("rowActions.deleting") : t("common.delete")}
              </Button>
            </div>
          </div>
        </Modal>
...
function TurnCard({ turn, t }: { turn: RunResult; t: Translate }) {
  return (
    <div className="space-y-2">
      <div className="flex justify-end">
        <div className="max-w-[85%] rounded-2xl rounded-ee-sm bg-blue-600 px-4 py-2 text-sm text-white">
          {turn.goal}
        </div>
      </div>
      <Card>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="text-sm font-semibold text-slate-800">{t("ag.answer")}</span>
          <Badge tone={turn.status === "completed" ? "green" : "amber"}>{enumLabel(turn.status, t)}</Badge>
          <span className="text-xs text-slate-400">{t("ag.toolSteps", { n: turn.step_count })}</span>
          {turn.skill_used && (
            <span className="inline-flex items-center gap-1 text-xs font-medium text-violet-600">
              <Repeat size={12} /> {t("ag.reusedSkill", { name: turn.skill_used })}
            </span>
          )}
          {turn.skill_created && (
            <span className="inline-flex items-center gap-1 text-xs font-medium text-amber-600">
              <Sparkles size={12} /> {t("ag.learnedSkill", { name: turn.skill_created })}
            </span>
          )}
      ...
      <div>
        <h3 className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-slate-500">
          <Wrench size={12} /> {t("ag.trajectory")}
        </h3>
        <div className="space-y-2">
          {turn.steps.map((step) => (
            <StepCard key={step.index} step={step} t={t} />
          ))}
        </div>
      </div>
    </div>
...
export default function Agent() {
  ...
  if (!canRun) {
    return (
      <div>
        <PageHeader title={t("ag.title")} />
        <Card>
          <EmptyState message={t("ag.restricted")} />
        </Card>
      </div>
    );
  }
  ...
  return (
    <div>
      <PageHeader title={t("ag.title")} subtitle={t("ag.subtitle")} />

      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        <div className="flex h-[calc(100vh-220px)] min-h-[420px] flex-col lg:col-span-2">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-xs text-slate-400">
              {conversationId ? t("ag.conversationN", { id: conversationId }) : t("ag.newConversationLabel")}
            </span>
            <div className="flex items-center gap-3">
              ...
              >
                <HistoryIcon size={13} /> {t("ag.history")}
              </button>
              {turns.length > 0 && (
                <button
                  ...
                  className="inline-flex items-center gap-1 text-xs font-medium text-slate-500 hover:text-slate-700"
                >
                  <MessageSquarePlus size={13} /> {t("ag.newConversation")}
                </button>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 space-y-4 overflow-y-auto rounded-xl border border-slate-200 bg-white p-4">
            {turns.length === 0 && !busy && (
              <div className="flex h-full flex-col items-center justify-center gap-2 text-center text-slate-400">
                <Bot size={36} />
                <p className="text-sm">{t("ag.emptyHint1")}</p>
                <p className="text-xs">{t("ag.emptyHint2")}</p>
              </div>
            )}
            {turns.map((turn, i) => (
              <TurnCard key={turn.id ?? i} turn={turn} t={t} />
            ))}
            {busy && (
              <div className="flex items-center gap-2 ps-1 text-sm text-slate-400">
                <Spinner /> {t("ag.reasoning")}
              </div>
            )}
            <div ref={endRef} />
          </div>
          ...
          <form onSubmit={runAgent} className="mt-3 flex flex-wrap items-end gap-2">
            <Field label={t("ag.projectOptional")}>
              <div className="w-56">
                <ProjectPicker
                  projects={projects}
                  value={projectId}
                  onChange={setProjectId}
                  placeholder={t("ag.wholePortfolio")}
                />
              </div>
            </Field>
            <input
              ...
              placeholder={t("ag.goalPlaceholder")}
        ...
        <div>
          <h3 className="mb-2 flex items-center gap-1.5 text-sm font-semibold text-slate-700">
            <Sparkles size={14} className="text-amber-500" /> {t("ag.skillLibrary")}
            <span className="text-xs font-normal text-slate-400">({skills.length})</span>
          </h3>
          <div className="space-y-2">
            {skills.length === 0 && (
              <Card>
                <p className="text-sm text-slate-400">{t("ag.noSkills")}</p>
              </Card>
            )}
            {skills.map((skill) => (
              <div key={skill.id} className="space-y-1">
                <SkillCard skill={skill} isAdmin={isAdmin} onChanged={loadSkills} t={t} />
                {canRun && skill.status === "active" && (
                  <button
                    onClick={() => runSkill(skill)}
                    disabled={busy || goal.trim().length < 3}
                    className="inline-flex items-center gap-1 ps-1 text-xs font-medium text-blue-600 hover:text-blue-700 disabled:text-slate-300"
                  >
                    <PlayCircle size={13} /> {t("ag.runSkill")}
                  </button>
                )}
              </div>
            ))}
      ...
      {historyOpen && (
        <Modal title={t("ag.pastRuns")} onClose={() => setHistoryOpen(false)}>
          {historyBusy && history.length === 0 ? (
            <div className="flex justify-center py-6"><Spinner /></div>
          ) : history.length === 0 ? (
            <EmptyState message={t("ag.noPastRuns")} />
          ) : (
            <>
              <div className="space-y-2">
                {history.map((run) => (
                  <button
                    ...
                    className="block w-full rounded-lg border border-slate-200 p-3 text-start hover:border-blue-300 hover:bg-blue-50/40 disabled:opacity-50"
                  >
                    <div className="flex items-center gap-2">
                      <Badge tone={run.status === "completed" ? "green" : "amber"}>{enumLabel(run.status, t)}</Badge>
